...
const FilteredAccommodations = ({ submitted }) => {
  ...
  const hostCountry = searchParams.get("destination");
  const guests = searchParams.get("numberOfGuests");
  const amount = searchParams.get("budget");
  ...
  const fetchFilteredAccommodations = async () => {
    ...
    const response = await fetch(
      `http://localhost:4000/api/accommodation/filter/accommodations/?destination=${hostCountry}&numberOfGuests=${guests}&budget=${amount}&arrivalDate=${arrival}&departureDate=${departure}`
    );
...
</div> : <section className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 md:gap-8 mt-4">
        {accommodations &&
